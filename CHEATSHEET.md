...
Meets AC · implements the frozen contract · expand/contract migrations · tests green at the right
layer · app observed working · **CI green** · `code-review` + `simplify` clean (+ `security-review`
if sensitive) · docs updated (`architecture.md` / ADR) · tracker issue updated (status / closed).
...
- **Conventional Commits:** `type(scope): summary` (imperative, ≤72 chars). Types: `feat` `fix`
  `refactor` `test` `docs` `chore` `perf` `build` `ci`. Reference the issue (`Refs #123` / `Closes #123`).
- Small logical commits, not one blob. PRs small & reviewable, one feature per branch, squash-merge.
- **Where planning commits land:** Stage 0–3 docs (PRD, ADRs, frozen contract) are
